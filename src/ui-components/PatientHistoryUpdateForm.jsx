...
import { getPatientHistory } from "../graphql/queries";
import { updatePatientHistory } from "../graphql/mutations";
...
export default function PatientHistoryUpdateForm(props) {
  const {
    id: idProp,
    patientHistory: patientHistoryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = patientHistoryRecord
      ? { ...initialValues, ...patientHistoryRecord }
      : initialValues;
  ...
  const [patientHistoryRecord, setPatientHistoryRecord] = React.useState(
    patientHistoryModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getPatientHistory.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPatientHistory
        : patientHistoryModelProp;
      setPatientHistoryRecord(record);
    };
    queryData();
  }, [idProp, patientHistoryModelProp]);
  React.useEffect(resetStateValues, [patientHistoryRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePatientHistory.replaceAll("__typename", ""),
            variables: {
              input: {
                id: patientHistoryRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PatientHistoryUpdateForm")}
      {...rest}
    >
      <TextField
        label="First name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || patientHistoryModelProp)}
